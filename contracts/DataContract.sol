...
contract DataContract is AutherController {
    ...
    function changeCredaToken(address _creda) external onlyOwner {
        credaToken = IERC20(_creda);
    }

    function setCredaFee(uint256 _fee) external onlyOwner {
        credaFee = _fee;
    }
    ...
    function getAddressBySource(
        address _source
    ) public view returns (address[] memory) {
        return creditSource[_source];
    }

    function getBindLists(
        address _address
    ) public view returns (address[] memory) {
    ...
    function getDidByAddress(
        ...
        return
            _creditInfo[_address].didStatus == 0
                ? createDidByAddress(_address)
                : _creditInfo[_address].did;
    }

    function getCreditInfo(
        address _address
    ) public view returns (CreditInfo memory) {
    ...
    function creditDetail(
        ...
        return [
            bytesToUint(getByteByIndex(credit, 0)),
            bytesToUint(getByteByIndex(credit, 1)),
            bytesToUint(getByteByIndex(credit, 2)),
            bytesToUint(getByteByIndex(credit, 3)),
            bytesToUint(getByteByIndex(credit, 4)),
            bytesToUint(getByteByIndex(credit, 5)),
            bytesToUint(getByteByIndex(credit, 6)),
            bytesToUint(getByteByIndex(credit, 7)),
            bytesToUint(getByteByIndex(credit, 8)),
            bytesToUint(getByteByIndex(credit, 9)),
            bytesToUint(getByteByIndex(credit, 10)),
            bytesToUint(getByteByIndex(credit, 11)),
            bytesToUint(getByteByIndex(credit, 12)),
            bytesToUint(getByteByIndex(credit, 13)),
            bytesToUint(getByteByIndex(credit, 14)),
    ...
    function updateCredit(
        ...
        require(
            checkStatus(_address, credit, merkleProof),
            "Merkle verify error"
        );
        if (credaFee > 0) {
    ...
    function createDidByAddress(
        address _address
    ) public pure returns (bytes memory) {
        bytes memory newbytes = new bytes(32);
        bytes32 keccak = keccak256(
            abi.encodePacked(ManagerString.toAsciiString(_address))
        );
        for (uint i = 0; i < 32; i++) {
            newbytes[i] = keccak[i];
        }
    ...
    function unBindAddress(address mainAddress) public returns (bool) {
        require(
            mainAddress != address(0x0) &&
                addressBinds[msg.sender] == mainAddress,
            "user unbind main address "
        );
    ...
    function batchUpdateAllCredit(
        address[] calldata _address,
        uint256[] calldata _credits
    ) external onlyAuthorizedNodes {
